...
- **Pasarela de pagos: confirmada (actualización 2026-09-13).** Se completó un checkout de prueba real en `iber.uy` y se confirmó que el cobro pasa por **PlaceToPay** (proveedor de pagos regional), que agrupa varios medios (tarjetas Visa/Mastercard/Amex vía Scotiabank) y, al elegir "eBROU", redirige directamente a la plataforma real del banco (`ebanking.brou.com.uy`) para autorizar el pago ahí. Ya no es una pregunta abierta — solo confirmar en la entrevista si hay algo de conciliación/facturación adicional que maneje Montevideo COMM por fuera de eso.
...
- **Sistema de cobros a clientes** (tu pregunta original) — ✅ **Ya confirmado por prueba real de checkout:** la pasarela es PlaceToPay, que redirige a cada banco/tarjeta para autorizar (ver hallazgos arriba). Lo que queda genuino para preguntar: ¿hay algo de conciliación, facturación o cobranza que Montevideo COMM maneje por fuera de esa integración (dentro o fuera del ERP)?
